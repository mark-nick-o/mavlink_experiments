...
namespace domain
{
    ...
    private:
        ...
        std::uint32_t firmware_version;                                                     /*<  Version of the camera firmware (v << 24 & 0xff = Dev, v << 16 & 0xff = Patch, v << 8 & 0xff = Minor, v & 0xff = Major)*/
        std::float32_t focal_length;                                                        /*< [mm] Focal length*/
        std::float32_t sensor_size_h;                                                       /*< [mm] Image sensor size horizontal*/
        std::float32_t sensor_size_v;                                                       /*< [mm] Image sensor size vertical*/
        std::uint32_t flags;                                                                /*<  Bitmap of camera capability flags.*/
        ...
        std::uint8_t vendor_name[32u];                                                      /*<  Name of the camera vendor*/
        std::uint8_t model_name[32u];                                                       /*<  Name of the camera model*/
        std::uint8_t lens_id;                                                               /*<  Reserved for a lens ID*/
        std::char cam_definition_uri[140u];  
