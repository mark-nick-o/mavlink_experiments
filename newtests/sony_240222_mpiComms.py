...
class sonyAlphaNewCamera():
    ...
    READ_TRIGGER = 0                                                                       #: setSonyCamISO_CheckTrigger_Master
    SEND_TO_MPI  = 1                                                                       #: sendISOReqOverMPI_Master
    RECV_FROM_MPI = 2                                                                      #: readISOResultOverMPI_Master
    WRITE_TO_OBJ = 3                                                                       #: setSonyCamISO_ChkResWrite_Master
    
    def __init__ (self, name = 'sonyAlphaCamClass'):
        self.name = name                                                                   # name as a string
        self.mav_cam_action_state = multiprocessing.Value('i', sonyAlphaNewCamera.READ_TRIGGER )
    ...
    def readStringReplyOverMPILink( rankId=1 ):
        ...
        # receive data the reply from the sony API as a string from the slot running the action
        #
        comm.Recv(data, source=rankId, tag=0)
        ...
        for s in range(0,len(list1)):
            ansStr = ansStr + chr(int(list1[s]))
        print(f"\033[32m MPI Process {rank} received the ansStr \033[31;46m {ansStr} \033[0m") 
        return ansStr
    #
    ...
    def doMPISlaveRcv( mavObj, stillcap, whitebal, shut_sp, iso, focusdata, focusarea, aper, expro ):
        ...
        if len(list1) > 1:
            ...
            # return list1[1]
            if tagName.find("S_ISO") == -1:
                self.setSonyCamISO_ChkResWrite_Master( iso, mavObj, list1, 5)
            elif tagName.find("S_APERTURE") == -1:
                self.setSonyCamISO_ChkResWrite_Master( aper, mavObj, list1, 2)
            elif tagName.find("S_WHITE_BAL") == -1:
                self.setSonyCamISO_ChkResWrite_Master( whitebal, mavObj, list1, 7)
            elif tagName.find("S_FOCUS_AREA") == -1:
                self.setSonyCamISO_ChkResWrite_Master( focusarea, mavObj, list1, 4)
            elif tagName.find("S_FOCUS_MODE") == -1:
                self.setSonyCamISO_ChkResWrite_Master( focusdata, mavObj, list1, 3)
            elif tagName.find("S_SHUT_SPD") == -1:
                self.setSonyCamISO_ChkResWrite_Master( shut_sp, mavObj, list1, 6)
            elif tagName.find("S_STILL_CAP") == -1:
                self.setSonyCamISO_ChkResWrite_Master( stillcap, mavObj, list1, 8)
            elif tagName.find("S_EX_PRO_MODE") == -1:
                self.setSonyCamISO_ChkResWrite_Master( expro, mavObj, list1, 1)
    ...
    def readTagOverMPILink( rankId=0 ):
        ...
        # receive data from master process rank=0 (first its the tag as a string)
        # receive data from slave rank1 process rank=1 (first its the tag as a string)
        #
        comm.Recv(data, source=rankId, tag=0)
    ...
    # READ_TRIGGER & LOOK FOR CHANGE (0)
    #
    # ------ Sequence of actions for a camera request to change --------
    #
    # READ_TRIGGER : setSonyCamISO_CheckTrigger_Master
    # SEND_TO_MPI  : sendISOReqOverMPI_Master
    # RECV_FROM_MPI : readISOResultOverMPI_Master
    # WRITE_TO_OBJ : setSonyCamISO_ChkResWrite_Master
    #
    def setSonyCam_CheckTrigger_Master( self, mavObj, index, timeout1=100, timeout2=50, no_timeout1_retry=1, no_timeout2_retry=1 ):

        if (self.mav_cam_action_state.value == sonyAlphaNewCamera.READ_TRIGGER):                                  # mav_cam_action_state = new state of what the camera needs to do
            ret = False
            readSuccess = False
            print(" =========== set sony cam iso ================ ")

            ee = -1                                                                     # return the enumeration as -1 if its not possible        
            # 
            timeoutS1 = timeout1 * no_timeout1_retry
            ...
            if ((not (int(reqDat) == mavlinkSonyCamWriteVals.STATE_INIT) and not (int(reqDat) == int(prevDat))) and (readSuccess == True)):
                ...
                if (ret == False):
                    ...
                    return -1
                print(f"enumeration value for iso {ee} req {reqDat}")
                # MAKE CHANGE and PREPARE A SEND FROM FROM RANK0 TO CAMERA RANK1 
                with self.mav_cam_action_state.get_lock():
                    self.mav_cam_action_state.value == sonyAlphaNewCamera.SEND_TO_MPI
                return ee
    ...
    def sendReqOverMPI_Master( enumSetVal, index ):
        ...
        if (self.mav_cam_action_state.value == sonyAlphaNewCamera.SEND_TO_MPI):
            ...
            float_list = self.makeMPISendList(enumSetVal,index)
            self.sendValueListOverMPILink( float_list )
            with self.mav_cam_action_state.get_lock():            
                self.mav_cam_action_state.value == sonyAlphaNewCamera.RECV_FROM_MPI            
    ...
    def readISOResultOverMPI_Master( mavObj ):
        ...
        if (self.mav_cam_action_state.value == sonyAlphaNewCamera.RECV_FROM_MPI): 
            self.doMPISlaveRcv()
            #with self.mav_cam_action_state.get_lock():            
            #    self.mav_cam_action_state.value == sonyAlphaNewCamera.WRITE_TO_OBJ
            
    # returns true : action completed
    # returns false : then resent the MPI request
    # returns None : invalid state
    #
    def setSonyCamISO_ChkResWrite_Master( self, mem, mavObj, mpi_list, index, timeout1=100, timeout2=50, no_timeout1_retry=1, no_timeout2_retry=1 ):         

        ret = None
        if (self.mav_cam_action_state.value == sonyAlphaNewCamera.RECV_FROM_MPI):    ### not sure we need to care about the state ??????????
            ret = False        
            ans = mpi_list
            # PROCESS RANK1_REPLY i.e what the camera action replied with and check it against the mavlink request
            if not (ans is None):   
                writeSuccess = False 
                wpWrite = False        
                ...
                readSuccess = False
                print(" =========== set sony cam iso ================ ")
                # return the enumeration as -1 if its not possible        
                # 
                timeoutS1 = timeout1 * no_timeout1_retry

                # READ_TRIGGER TO SEE REQUEST
                while (readSuccess == False) and (timeoutS1 > 0):
                    if (index == 5):
                        reqDat, prevDat, readSuccess  = mavObj.getVal_sony_iso(mavObj.STATE_CAM_READING,timeout1)
                    elif (index == 2):
                ...
                if (len(ans)==0):
                    print("setting iso the length of command return was zero")
                    with self.mav_cam_action_state.get_lock():            
                        self.mav_cam_action_state.value == sonyAlphaNewCamera.READ_TRIGGER                                       # <<< error handler on number of retries
                    return ret 
                if (len(ans)==4):                                                                                          # thats what we return for a non-writable value
                    if (((ans[0] == -1) and (ans[1] == -1)) and (ans[2] == -1)):                                           # if we get that we cant write it, we reset the request                   
                        if (index == 5):
                            writeSuccess = mavObj.clearReq_sony_iso( mavObj.STATE_CAM_WRITING )
                            while wpWrite == False:
                        ...
                        with self.mav_cam_action_state.get_lock():            
                            self.mav_cam_action_state.value == sonyAlphaNewCamera.READ_TRIGGER
                        return writeSuccess
                print(f" \033[32m set the ISO from/to :: {ans} \033[0m")                       
                try:
                    if ( int(ans[1]) == int(reqDat) ) :
                        while (writeSuccess == False) and (timeoutS2 > 0):  
                            if (index == 5):
                                ...
                                writeSuccess = mavObj.setVal_sony_aperture( int(ans[1]), mavObj.STATE_CAM_WRITING, mavObj.WRITE_PREV_DATA, timeout2 ) 
                            elif (index == 7):
                                writeSuccess = mavObj.setVal_sony_white_bal( int(ans[1]), mavObj.STATE_CAM_WRITING, mavObj.WRITE_PREV_DATA, timeout2 ) 
                            elif (index == 1):
                                writeSuccess = mavObj.setVal_sony_ex_pro( int(ans[1]), mavObj.STATE_CAM_WRITING, mavObj.WRITE_PREV_DATA, timeout2 ) 
                            elif (index == 8):
                                writeSuccess = mavObj.setVal_sony_still_cap_mode( int(ans[1]), mavObj.STATE_CAM_WRITING, mavObj.WRITE_PREV_DATA, timeout2 ) 
                            elif (index == 6):
                                writeSuccess = mavObj.setVal_sony_shutter( int(ans[1]), mavObj.STATE_CAM_WRITING, mavObj.WRITE_PREV_DATA, timeout2 ) 
                            elif (index == 3):
                                writeSuccess = mavObj.setVal_sony_focus( int(ans[1]), mavObj.STATE_CAM_WRITING, mavObj.WRITE_PREV_DATA, timeout2 ) 
                            elif (index == 4):
                                writeSuccess = mavObj.setVal_sony_focus_area( int(ans[1]), mavObj.STATE_CAM_WRITING, mavObj.WRITE_PREV_DATA, timeout2 )                             
                            timeoutS2 -= timeout2                                  
                            print(f" write {writeSuccess}")
                    else:                                                                                                  # <<< error handler on number of retries
                        print(f" what wat {ans[1]}=={reqDat}")
                except Exception as err_msg:                
                   print("\033[31m write sony iso failed to set iso \033[0m")                    
                if ( writeSuccess == True ):
...
#
def rundoMPIRcv( mySonyCamNo1, gcsWrites2Sony, stillcap, whitebal, shut_sp, iso, focusdata, focusarea, aper, expro ):
    while True:
        mySonyCamNo1.doMPISlaveRcv( gcsWrites2Sony, stillcap, whitebal, shut_sp, iso, focusdata, focusarea, aper, expro )
...
if __name__ == '__main__':
    ...
    if (rank == 0):         # ====================== master rank 0 ==========================================================
        ...
        # 
        schedule = list(range(1,9))

        # runs the MPI reciever from rank 1 as a multi-process daemon always running in the background
        #
        p1 = multiprocessing.Process(name='rundoMPIRcv', target=rundoMPIRcv, args=(mySonyCamNo1, gcsWrites2Sony, stillcap, whitebal, shut_sp, iso, focusdata, focusarea, aper, expro,))   
        p1.daemon = True
        ...
        while active==True:
            ...
            # check each option for writes from mavlink, if seen then send data over mpi link 
            # to process slot rank1 to perfrom the action on the camera over usb link
            #
            for index in schedule:
                if not (gcsWrites2Sony.set_sony_iso.value == gcsWrites2Sony.STATE_INIT) and (index == 5):
                    reqValue = mySonyCamNo1.setSonyCam_CheckTrigger_Master( gcsWrites2Sony, index )
                    if not (reqValue == -1): 
                        mySonyCamNo1.sendReqOverMPI_Master( reqValue, index )
                elif not (gcsWrites2Sony.set_sony_aperture.value == gcsWrites2Sony.STATE_INIT) and (index == 2):
                    reqValue = mySonyCamNo1.setSonyCam_CheckTrigger_Master( gcsWrites2Sony, index )
                    if not (reqValue == -1): 
                        mySonyCamNo1.sendReqOverMPI_Master( reqValue, index )
                elif not (gcsWrites2Sony.set_sony_white_bal.value == gcsWrites2Sony.STATE_INIT) and (index == 7):
                    reqValue = mySonyCamNo1.setSonyCam_CheckTrigger_Master( gcsWrites2Sony, index )
                    if not (reqValue == -1): 
                        mySonyCamNo1.sendReqOverMPI_Master( reqValue, index )
                elif not (gcsWrites2Sony.set_sony_ex_pro.value == gcsWrites2Sony.STATE_INIT) and (index == 1):
                    reqValue = mySonyCamNo1.setSonyCam_CheckTrigger_Master( gcsWrites2Sony, index )
                    if not (reqValue == -1): 
                        mySonyCamNo1.sendReqOverMPI_Master( reqValue, index )
                elif not (gcsWrites2Sony.set_sony_still_cap_mode.value == gcsWrites2Sony.STATE_INIT) and (index == 8):
                    reqValue = mySonyCamNo1.setSonyCam_CheckTrigger_Master( gcsWrites2Sony, index )
                    if not (reqValue == -1): 
                        mySonyCamNo1.sendReqOverMPI_Master( reqValue, index )
                elif not (gcsWrites2Sony.set_sony_shutter.value == gcsWrites2Sony.STATE_INIT) and (index == 6):
                    reqValue = mySonyCamNo1.setSonyCam_CheckTrigger_Master( gcsWrites2Sony, index )
                    if not (reqValue == -1): 
                        mySonyCamNo1.sendReqOverMPI_Master( reqValue, index )
                elif not (gcsWrites2Sony.set_sony_focus.value == gcsWrites2Sony.STATE_INIT) and (index == 3):
                    reqValue = mySonyCamNo1.setSonyCam_CheckTrigger_Master( gcsWrites2Sony, index )
                    if not (reqValue == -1): 
                        mySonyCamNo1.sendReqOverMPI_Master( reqValue, index )
                elif not (gcsWrites2Sony.set_sony_focus_area.value == gcsWrites2Sony.STATE_INIT) and (index == 4):
                    reqValue = mySonyCamNo1.setSonyCam_CheckTrigger_Master( gcsWrites2Sony, index )
                    if not (reqValue == -1): 
                        mySonyCamNo1.sendReqOverMPI_Master( reqValue, index )
        ...
        while (loop == True): 
            ...
            if len(list1) > 1:
                ...
                if (list1[1] == 5) and not (tagName.find("S_ISO") == -1):
                    ...
                    data = np.array(ansList, dtype="float64")
                    comm.Send(data, dest=0, tag=0)    
                elif (list1[1] == 4) and not (tagName.find("S_FOCUS_AREA") == -1) :
                    ansList = mySonyAlpha.set_sony_focus(list1[0])  
                    print(f"FOCUS AREA ran in the shell with a reply of {ansList}")                    
                    data = np.array(ansList, dtype="float64")
                    comm.Send(data, dest=0, tag=0) 
                elif (list1[1] == 3) and not (tagName.find("S_FOCUS_MODE") == -1) :
                    ansList = mySonyAlpha.set_sony_focus_area(list1[0])  
                    print(f"FOCUS MODE ran in the shell with a reply of {ansList}")                    
                    data = np.array(ansList, dtype="float64")
                    comm.Send(data, dest=0, tag=0) 
                elif (list1[1] == 6) and not (tagName.find("S_SHUT_SPD") == -1) :
                    ansList = mySonyAlpha.set_sony_shutter(list1[0])  
                    print(f"SHUTTER SPEED ran in the shell with a reply of {ansList}")                    
                    data = np.array(ansList, dtype="float64")
                    comm.Send(data, dest=0, tag=0) 
                elif (list1[1] == 8) and not (tagName.find("S_STILL_CAP") == -1) :
                    ansList = mySonyAlpha.set_sony_still_cap(list1[0])  
                    print(f"STILL CAPTURE ran in the shell with a reply of {ansList}")                    
                    data = np.array(ansList, dtype="float64")
                    comm.Send(data, dest=0, tag=0)   
                elif (list1[1] == 1) and not (tagName.find("S_EX_PRO_MODE") == -1) :
                    ansList = mySonyAlpha.set_sony_ex_pro(list1[0])  
                    print(f"EXPOSURE PROGRAM ran in the shell with a reply of {ansList}")                    
                    data = np.array(ansList, dtype="float64")
                    comm.Send(data, dest=0, tag=0) 
