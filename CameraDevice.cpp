...
void CameraDevice::set_still_file_format()
{
    load_properties();
    std::int32_t mode = 3;
    SDK::CrError err = SDK::SendCommand(m_device_handle, (SDK::CrCommandId)SDK::CrDevicePropertyCode::CrDeviceProperty_FileType, (SDK::CrCommandParam)mode);
...
void CameraDevice::load_properties()
{
    ...
    if (CR_FAILED(status)) {
        tout << "Load Props :: Failed to get device properties.\n";
        return;
    }
