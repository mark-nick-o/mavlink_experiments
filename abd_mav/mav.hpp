...
#include "defs.hpp"
#include "/home/anthony/code/mavlink-router/include/mavlink/common/mavlink.h"
#include "/home/anthony/code/mavlink-router/include/mavlink/common/common.h"
#include <arpa/inet.h>
#include <errno.h>
#include <fcntl.h>
#include <signal.h>
...
namespace mlr
{
  ...
  local_server_socket::local_server_socket(bool coot,bool tcp, char *_ip, int _port)
  {
        ...
        this->port = _port;
            ...
            if(tcp)  strncpy(this->info.IP_protocal,"TCP",3);
            if(!tcp) strncpy(this->info.IP_protocal,"UDP",3);    
            strncpy(this->ip,_ip,strlen(_ip));
...
/**************************************************************************************************/
    ...
    // lint complained so i corrected this for types and uninitialised variables (nothing sles)
    heartbeat::heartbeat(float _frequency_hZ=1.0f)
    { 
        //#error "to do haert to socket and auto send"
        if (0.0f >= _frequency_hZ) {
           std::cout << "frequency Hz cannot be negative or zero " << _frequency_hZ << std::endl;
        } else {
           /*long double nsec = 0.0f; */
           unsigned int sec = 0;
           this->timeout.tv_sec =0;
           long double nsec = (1000000000.0f / static_cast<long double>(_frequency_hZ));
           if(nsec >= 1000000000.0f)
           {
              sec = static_cast<unsigned int>(nsec / 1000000000.0f);
              nsec -= (1000000000.0f*static_cast<long double>(sec));
           }
