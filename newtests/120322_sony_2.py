...
class mavlinkSonyCamWriteVals():
    ...
    # --- sony camera ---
    ParamStillCap = 1                       # 2**0 1<<0
    ParamWhiteBala = 2                      # 2**1 1<<1
    ParamShutSpd = 4                        # 2**2 1<<2
    ParamIso = 8                            # 2**3 1<<3 etc
    ParamFocus = 16
    ParamFocusArea = 32
    ParamAperture = 64
    ParamExPro = 128
    sony_pars = [ ParamStillCap, ParamWhiteBala, ParamShutSpd, ParamIso, ParamFocus, ParamFocusArea, ParamAperture, ParamExPro ]
    # --- micasense camera ---
    ParamExPos1 = 2**8
    ParamExPos2 = 2**9
    ParamExPos3 = 2**10
    ...
    total_sony = (ParamStillCap | ParamWhiteBala | ParamShutSpd | ParamIso | ParamFocusArea | ParamFocus | ParamAperture | ParamExPro)    
    ...
    total_mica = (all_mica_expsoure | ParamRedShut | ParamRedMode)
    alpha7_options = (total_sony | ParamCamChoice)
    mica_options = ((total_mica<<len(sony_pars)) | ParamCamChoice)
    ...
    def __init__ (self):
        ...
        self.set_mica_shutter = multiprocessing.Value('i', mavlinkSonyCamWriteVals.STATE_INIT)
        self.set_mica_mode = multiprocessing.Value('i', mavlinkSonyCamWriteVals.STATE_INIT)
        ...
        self.prev_mica_shutter = multiprocessing.Value('i', mavlinkSonyCamWriteVals.STATE_INIT)
        self.prev_mica_mode = multiprocessing.Value('i', mavlinkSonyCamWriteVals.STATE_INIT)
    ...
    def setVal_mica_mode(self,value,myId,mode=0,timeout=20,reset_state=False):
        timeCnt = 0
        while (not (self.state.value == mavlinkSonyCamWriteVals.STATE_READY)) and (timeCnt < timeout):
            time.sleep(0.1)
            timeCnt += 1
        ...
        if (timeCnt < timeout):
            with self.state.get_lock():
                self.state.value = myId
            if mode == 1:
                with self.set_mica_mode.get_lock():
                    self.prev_mica_mode.value = self.set_mica_mode.value
            with self.set_mica_mode.get_lock():
                self.set_mica_mode.value = int(value)
            with self.state.get_lock():
                self.state.value = mavlinkSonyCamWriteVals.STATE_READY
            return True
        else:
    ...
    def clearReq_mica_mode(self,myId,timeout=20,reset_state=False):
        timeCnt = 0
        while (not (self.state.value == mavlinkSonyCamWriteVals.STATE_READY)) and (timeCnt < timeout):
            time.sleep(0.1)
            timeCnt += 1
        ...
        if (timeCnt < timeout):
            with self.state.get_lock():
                self.state.value = myId
            with self.set_mica_mode.get_lock():
                self.set_mica_mode.value = self.prev_mica_mode.value
            with self.state.get_lock():
                self.state.value = mavlinkSonyCamWriteVals.STATE_READY
            return True
        else:
    ...
    def getVal_mica_mode(self, YourID, timeout=20, reset_state=False):  
        timeCnt = 0
        while (not (self.state.value == mavlinkSonyCamWriteVals.STATE_READY)) and (timeCnt < timeout):
            time.sleep(0.1)
            timeCnt += 1
        ...
        if (timeCnt < timeout):
            with self.state.get_lock():
                self.state.value = YourID
            print('value: {} previous: {}'.format(self.set_mica_mode.value, self.prev_mica_mode.value))
            c = self.set_mica_mode.value
            p = self.prev_mica_mode.value
            with self.state.get_lock():
                self.state.value = mavlinkSonyCamWriteVals.STATE_READY
            return c, p, True
        else:
            if (reset_state == True):
                with self.state.get_lock():            
                    self.state.value = mavlinkSonyCamWriteVals.STATE_READY
            return self.set_mica_mode.value, self.prev_mica_mode.value, False
            
    def setVal_mica_shutter_sp(self, value, myId, mode=0, timeout=20, reset_state=False):
        timeCnt = 0
        while (not (self.state.value == mavlinkSonyCamWriteVals.STATE_READY)) and (timeCnt < timeout):
            time.sleep(0.1)
            timeCnt += 1
        ...
        if (timeCnt < timeout):
            with self.state.get_lock():
                self.state.value = myId
            if mode == 1:
                with self.set_mica_shutter.get_lock():
                    self.prev_mica_shutter.value = self.set_mica_shutter.value
            with self.set_mica_shutter.get_lock():
                self.set_mica_shutter.value = int(value)
            with self.state.get_lock():
                self.state.value = mavlinkSonyCamWriteVals.STATE_READY
            return True
        else:
    ...
    def clearReq_mica_shutter_sp(self, myId, timeout=20, reset_state=False):
        timeCnt = 0
        while (not (self.state.value == mavlinkSonyCamWriteVals.STATE_READY)) and (timeCnt < timeout):
            time.sleep(0.1)
            timeCnt += 1
        ...
        if (timeCnt < timeout):
            with self.state.get_lock():
                self.state.value = myId
            with self.set_mica_shutter.get_lock():
                self.set_mica_shutter.value = self.prev_mica_shutter.value
            with self.state.get_lock():
                self.state.value = mavlinkSonyCamWriteVals.STATE_READY
            return True
        else:
    ...
    def getVal_mica_shutter_sp(self, YourID, timeout=20, reset_state=False):  
        timeCnt = 0
        while (not (self.state.value == mavlinkSonyCamWriteVals.STATE_READY)) and (timeCnt < timeout):
            time.sleep(0.1)
            timeCnt += 1
        ...
        if (timeCnt < timeout):
            with self.state.get_lock():
                self.state.value = YourID
            print('value: {} previous: {}'.format(self.set_mica_shutter.value, self.prev_mica_shutter.value))
            c = self.set_mica_shutter.value
            p = self.prev_mica_shutter.value
            with self.state.get_lock():
                self.state.value = mavlinkSonyCamWriteVals.STATE_READY
            return c, p, True
        else:
            if (reset_state == True):
                with self.state.get_lock():            
                    self.state.value = mavlinkSonyCamWriteVals.STATE_READY
            return self.set_mica_shutter.value, self.prev_mica_shutter.value, False
    ...
    def setMavMicaModeData( self, dataRcv ):
    
        ret = False               
        ret = self.setVal_mica_mode(dataRcv, mavlinkSonyCamWriteVals.STATE_MAV_WRITING, mavlinkSonyCamWriteVals.DONT_WRITE_PREV_DATA, 5) 
        return ret

    def getMavMicaModeData( self ):
    
        ret = False  
        set_sony = 0
        prev_sony = 0        
        set_sony, prev_sony, ret = self.getVal_mica_mode(mavlinkSonyCamWriteVals.STATE_MAV_READING) 
        return set_sony, prev_sony, ret
        
    def setMavMicaShutterSpeedData( self, dataRcv ):
    
        ret = False               
        ret = self.setVal_mica_shutter_sp(dataRcv, mavlinkSonyCamWriteVals.STATE_MAV_WRITING, mavlinkSonyCamWriteVals.DONT_WRITE_PREV_DATA, 5) 
        return ret

    def getMavMicaShutterSpeedData( self ):
    
        ret = False  
        set_sony = 0
        prev_sony = 0        
        set_sony,prev_sony,ret = self.getVal_mica_shutter_sp(mavlinkSonyCamWriteVals.STATE_MAV_READING) 
        return set_sony, prev_sony, ret
...
#
    ...
    def initMicaModeData( self, mode_v ):
    
        ans = mode_v                                                                                        # we can only read back its not in manual ?
        if not (ans is None):
            print(f" MicaSense Mode =  {ans}")
            try:
                MicaObject = memoryValue('RED_MODE',int(ans[0]))
                with MicaObject.updateNeeded.get_lock():
                    MicaObject.updateNeeded.value = True  
            except Exception as err_msg:
                print("\033[31m Failed set the object to initial value : %s \033[0m" % (err_msg))
                MicaObject = memoryValue('RED_MODE',0)  
        else:
            print("\033[31m Cant get MicaSense Camera Mode \033[0m")
            MicaObject = memoryValue('RED_MODE',0)   

        MicaObject.index = memoryValue.numberOfVals 
        print(f"MicaSense Camera Mode : created object number : {MicaObject.index}")            
        return MicaObject 
        
    def initMicaShutSpeedData( self ):
    
        ans = 0                                                                                        # TODO dont think we can read back the shutter speed settings ?
        if not (ans is None):
            print(f" MicaSense Shutter Speed =  {ans}")
            try:
                MicaObject = memoryValue('RED_SHUTTERSPD',int(ans[0]))
                with MicaObject.updateNeeded.get_lock():
                    MicaObject.updateNeeded.value = True  
            except Exception as err_msg:
                print("\033[31m Failed set the object to initial value : %s \033[0m" % (err_msg))
                MicaObject = memoryValue('RED_SHUTTERSPD',0)  
        else:
            print("\033[31m Cant get MicaSense Shutter Speed \033[0m")
            MicaObject = memoryValue('RED_SHUTTERSPD',0)   

        MicaObject.index = memoryValue.numberOfVals 
        print(f"MicaSense Shutter Speed : created object number : {MicaObject.index}")            
        return MicaObject
...
class MAVFrame():
    ...
    def writeParamSetFromMavLink( self, msgString, mavObj, dataRcv, the_connection ):
        ...
        elif not (msgString.find("RED_MODE") == -1):
        #elif (re.search(patternStCa, msgString.upper())==True): 
            if (mavObj.setMavMicaModeData( dataRcv )==True):        
                return (self.mavlink_send_param_value_tag(the_connection, dataRcv, "RED_MODE")) 
        elif not (msgString.find("RED_SHUTTERSPD") == -1):
        #elif (re.search(patternStCa, msgString.upper())==True): 
            if (mavObj.setMavMicaShutterSpeedData( dataRcv )==True):        
                return (self.mavlink_send_param_value_tag(the_connection, dataRcv, "RED_SHUTTERSPD"))                 
    ...
    def readParamSetFromMavLink( self, msgString, mavObj, the_connection ):
        ...
        #elif (re.search(patternStCa, msgString.upper())==True): 
            with mavObj.mav_req_all_param.get_lock():        
                mavObj.mav_req_all_param.value |= mavlinkSonyCamWriteVals.ParamRedMode                                   # >>> set the bit to enable full read of parameter          
            v, p, r = mavObj.getMavMicaModeData( )
            if (r == True):
                return ( self.mavlink_send_param_value_tag(the_connection, v, "RED_MODE") )    
            else:
                return False
        elif not (msgString.find("RED_SHUTTERSPD") == -1):
        #elif (re.search(patternStCa, msgString.upper())==True): 
            with mavObj.mav_req_all_param.get_lock():        
                mavObj.mav_req_all_param.value |= mavlinkSonyCamWriteVals.ParamRedShut                                   # >>> set the bit to enable full read of parameter          
            v, p, r = mavObj.getMavMicaShutterSpeedData( )
            if (r == True):
                return ( self.mavlink_send_param_value_tag(the_connection, v, "RED_SHUTTERSPD") )    
            else:
                return False                
        elif not (msgString.find("RED_EXP_1") == -1):
        #elif (re.search(patternExPro, msgString.upper())==True): 
    ...
    def writeParamExtSetFromMavLink( self, msgString, mavObj, dataRcv, the_connection ):
        ...
        #elif (re.search(patternStCa, msgString.upper())==True):    
            if ((mavObj.setMavMicaModeData( dataRcv )) == True):
                    ret = self.mavlink_send_param_ext_ack( the_connection, dataRcv, mavdefs.PARAM_ACK_ACCEPTED, "RED_MODE" )
            else:
                ret = self.mavlink_send_param_ext_ack( the_connection, dataRcv, mavdefs.PARAM_ACK_FAILED, "RED_MODE" )  
        elif not (msgString.find("RED_SHUTTERSPD") == -1):
        #elif (re.search(patternStCa, msgString.upper())==True):    
            if ((mavObj.setMavMicaShutterSpeedData( dataRcv )) == True):
                    ret = self.mavlink_send_param_ext_ack( the_connection, dataRcv, mavdefs.PARAM_ACK_ACCEPTED, "RED_SHUTTERSPD" )
            else:
                ret = self.mavlink_send_param_ext_ack( the_connection, dataRcv, mavdefs.PARAM_ACK_FAILED, "RED_SHUTTERSPD" )  
        elif not (msgString.find("RED_EXP_1") == -1):
        #elif (re.search(patternStCa, msgString.upper())==True): 
            if (mavObj.setMavMicaExposureData( dataRcv, 1 )==True):        
    ...
    def readParamExtSetFromMavLink( self, msgString, mavObj, the_connection ):
        ...
        if not (msgString.find("CAM_ISO") == -1):
            ...
            v, p, r = mavObj.getMavMicaShutterSpeedData( )
            if (r == True):
                if ( self.mavlink_send_param_ext_value_tag(the_connection, v, "RED_SHUTTERSPD" ) == True ): 
                    with mavObj.mav_ext_req_all_param.get_lock():                 
                        mavObj.mav_ext_req_all_param.value |= mavObj.ParamRedShut                                # >>> set the bit to enable full read of parameter                  
                    return r
            return False    
        elif not (msgString.find("RED_MODE") == -1):
        #elif (re.search(patternStCa, msgString.upper())==True):   
            v, p, r = mavObj.getMavMicaModeData( )
            if (r == True):
                if ( self.mavlink_send_param_ext_value_tag(the_connection, v, "RED_MODE" ) == True ): 
                    with mavObj.mav_ext_req_all_param.get_lock():                 
                        mavObj.mav_ext_req_all_param.value |= mavObj.ParamRedMode                                # >>> set the bit to enable full read of parameter                  
                    return r
            return False             
        elif not (msgString.find("RED_EXP_1") == -1):
        #elif (re.search(patternStCa, msgString.upper())==True):   
...
# ------------------- asyncio wrappers for serial communication to the micasense camera -----------------------------------
#
#  TODO :: Consider also writing as a simple python shell to be occassionaly used
#          not sure if this will slow this app down or indeed we need to keep the connection 
#          and port read/write using a ping, if so may be simpler just to run a subprocess with a simple 
#          send of message and close port again. - To be tested
#  
async def read_mica_heart(the_connection): 
    """read micasense port for a heartbeat"""
    msg = the_connection.recv_match(type='HEARTBEAT', blocking=True)
    return msg
    
async def wait_heart_then_digicam_shutter(frame, the_connection, shutter_speed_req, attempts = 2):
    """wait for a heartbeat in 2Hz and then do digicam or report error."""
    print("\033[32m Sending to MicaSense over serial usb link ! \033[0m")
    timeout_s = 2.0                                                                             # timeout reading a heartbeat (should be 1Hz) 
    result = None
    while attempts >= 1:
        try:
            result = await asyncio.wait_for(read_mica_heart(the_connection), timeout=timeout_s)
        except asyncio.TimeoutError:
            print('\033[32m No heartbeat from micaSense camera in 2secs (0.5Hz) ! \033[0m')
            attempts -= 1
        if result is not None:
            frame.mavlink_do_digicam_configure_old(the_connection, shutter_speed=shutter_speed_req)
            attempts = 0
            print(f"\033[32m Completed the send of shutter speed {shutter_speed_req} for micaSense cam  \033[0m")

async def wait_heart_then_digicam_mode(frame, the_connection, mode_req, shutter_speed_req, attempts = 2):
    """wait for a heartbeat in 2Hz and then do digicam or report error."""
    print("\033[32m Sending to MicaSense over serial usb link ! \033[0m")
    timeout_s = 2.0                                                                             # timeout reading a heartbeat (should be 1Hz) 
    result = None
    while attempts >= 1:
        try:
            ...
            attempts -= 1
        if result is not None:
            frame.mavlink_do_digicam_configure_old(the_connection, mode=mode_req, shutter_speed=shutter_speed_req)
            attempts = 0
            print(f"\033[32m Completed the send of mode digicam_configure {shutter_speed_req} {mode_req} for micaSense cam  \033[0m")
...
async def setRedShutterSpd( mav_class, serial_mica_handle, shutter_speed_req ): 
    ...
    # function may be redundant from above usage with timeouts this would block 
...
async def sendMavRedShutter( mySonyCam, shut_chosen, ConnID ): 
    """asyncio wrapper for sending camera chocie information over mavlink"""
    p = multiprocessing.current_process()
    print ('Starting Mavlink MicaSense shutter speed :', p.name, p.pid)   
    success = mySonyCam.sendMavlinkMessageForParamObject( shut_chosen, ConnID )  
    success = mySonyCam.sendMavlinkMessageForParamExtObject( shut_chosen, ConnID )     
    print ('Exiting Mavlink MicaSense shutter speed :', multiprocessing.current_process().name) #

async def sendMavRedMode( mySonyCam, mode_chosen, ConnID ): 
    """asyncio wrapper for sending camera chocie information over mavlink"""
    p = multiprocessing.current_process()
    print ('Starting Mavlink MicaSense camera mode :', p.name, p.pid)   
    success = mySonyCam.sendMavlinkMessageForParamObject( mode_chosen, ConnID )  
    success = mySonyCam.sendMavlinkMessageForParamExtObject( mode_chosen, ConnID )     
    print ('Exiting Mavlink MicaSense camera mode :', multiprocessing.current_process().name) #
...
async def serviceParamRequests( mySonyCam, mav2SonyVals, stcap, wb, ss, iso, pf, pfa, pa, expro, redCam, re_expos, camera_choice, re_shut, re_m ):
    """wrapper for asyncio to service parameter change requests"""
    p = multiprocessing.current_process()
    ...
    if not (mav2SonyVals.mav_req_all_param.value == 0):
        ...
        if not ((int(mav2SonyVals.mav_req_all_param.value) & int(mav2SonyVals.ParamRedMode)) == 0):  
            re_m.set_update_flag( True, memoryValue.STATE_MAV_WRITING ) 

        if not ((int(mav2SonyVals.mav_req_all_param.value) & int(mav2SonyVals.ParamRedShut)) == 0):  
            re_shut.set_update_flag( True, memoryValue.STATE_MAV_WRITING ) 
    ...
    if not (mav2SonyVals.mav_ext_req_all_param.value == 0):
        ...
        if not ((int(mav2SonyVals.mav_ext_req_all_param.value) & int(mav2SonyVals.ParamRedMode)) == 0): 
            re_m.set_ack_send( True, memoryValue.STATE_MAV_WRITING ) 

        if not ((int(mav2SonyVals.mav_ext_req_all_param.value) & int(mav2SonyVals.ParamRedShut)) == 0): 
            re_shut.set_ack_send( True, memoryValue.STATE_MAV_WRITING ) 
        ...
        if not ((int(mav2SonyVals.mav_req_all_param.value) & int(mav2SonyVals.ParamCamChoice)) == 0):  
            if (mavlinkExtReqGetParam(  mySonyCam, camera_choice ) == True):
                with mav2SonyVals.mav_req_all_param.get_lock():
                    mav2SonyVals.mav_req_all_param.value = mav2SonyVals.mav_req_all_param.value & ~mav2SonyVals.ParamCamChoice  
        if not ((int(mav2SonyVals.mav_req_all_param.value) & int(mav2SonyVals.ParamRedMode)) == 0):  
            if (mavlinkExtReqGetParam(  mySonyCam, re_m ) == True):
                with mav2SonyVals.mav_req_all_param.get_lock():
                    mav2SonyVals.mav_req_all_param.value = mav2SonyVals.mav_req_all_param.value & ~mav2SonyVals.ParamRedMode 
        if not ((int(mav2SonyVals.mav_req_all_param.value) & int(mav2SonyVals.ParamRedShut)) == 0):  
            if (mavlinkExtReqGetParam(  mySonyCam, re_shut ) == True):
                with mav2SonyVals.mav_req_all_param.get_lock():
                    mav2SonyVals.mav_req_all_param.value = mav2SonyVals.mav_req_all_param.value & ~mav2SonyVals.ParamRedShut 
    ...
    if not (mav2SonyVals.mav_ext_req_all_param.value == 0):
        ...
        if not ((int(mav2SonyVals.mav_ext_req_all_param.value) & int(mav2SonyVals.ParamRedMode)) == 0): 
            if ( mavlinkExtReqGetParam( mySonyCam, re_m ) == True ):
                if ( re_m.set_ack_send( True, re_m.STATE_CAM_READING ) == True ):                                                      # additional PARAM_EXT_VALUE message requested               
                    with mav2SonyVals.mav_ext_req_all_param.get_lock():
                        mav2SonyVals.mav_ext_req_all_param.value = mav2SonyVals.mav_ext_req_all_param.value & ~mav2SonyVals.ParamRedMode

        if not ((int(mav2SonyVals.mav_ext_req_all_param.value) & int(mav2SonyVals.ParamRedShut)) == 0): 
            if ( mavlinkExtReqGetParam( mySonyCam, re_shut ) == True ):
                if ( re_shut.set_ack_send( True, re_shut.STATE_CAM_READING ) == True ):                                                      # additional PARAM_EXT_VALUE message requested               
                    with mav2SonyVals.mav_ext_req_all_param.get_lock():
                        mav2SonyVals.mav_ext_req_all_param.value = mav2SonyVals.mav_ext_req_all_param.value & ~mav2SonyVals.ParamRedShut
...
async def main():
    ...
    if expos_json is not None:
        ...
        red_edge_exposure.addNode("RED_EXP_1", 0)
        red_edge_exposure.addNode("RED_EXP_2", 0)
        red_edge_exposure.addNode("RED_EXP_3", 0)
        red_edge_exposure.addNode("RED_EXP_4", 0)
        red_edge_exposure.addNode("RED_EXP_5", 0)
        red_edge_exposure.addNode("RED_GAIN_1", 0)
        red_edge_exposure.addNode("RED_GAIN_2", 0)
        red_edge_exposure.addNode("RED_GAIN_3", 0)
        red_edge_exposure.addNode("RED_GAIN_4", 0)
        red_edge_exposure.addNode("RED_GAIN_5", 0)    
    for z in range(0, LinkedListMemory.numberOfVals):
        red_edge_exposure.set_update_flag(z, True)

    # other micasense objects for serial mavlink 
    if expos_json is not None:
        if expos_json['enable_man_exposure'] == True:
            m_mode = 4
        else:
            m_mode = 1
    else:
        m_mode = 0
    red_edge_mode = redEdgeCamNo1.initMicaModeData( m_mode )
    red_edge_shutter = redEdgeCamNo1.initMicaShutSpeedData( )
    ...
    while active==True:
        ...
        paramReqTask = asyncio.create_task(serviceParamRequests( mySonyCamNo1, gcsWrites2Sony, stillcap, whitebal, shut_sp, iso, focusdata, focusarea, aper, expro, redEdgeCamNo1, red_edge_exposure, camera_choice, red_edge_mode, red_edge_shutter ))
        ...
        # communicate on serial link to micasense camera if needed for adjusting shutter speed
        #
        if (not (gcsWrites2Sony.set_mica_shutter.value == gcsWrites2Sony.STATE_INIT) and not (gcsWrites2Sony.set_mica_shutter.value == gcsWrites2Sony.prev_mica_shutter.value)) and (lockout == True):
            print(f"\033[35m on TOP LEVEL saw micaSEnse shutter speed {gcsWrites2Sony.set_mica_shutter.value} {gcsWrites2Sony.prev_mica_shutter.value} {gcsWrites2Sony.mav_req_all_param.value} \033[0m")
            # send a heartbeat that says we are going into standby until we complete the camera action
            heartTask = asyncio.create_task(sendMavlinkHeartBeat_standby(frame, cID, 0)) 
            doRedShutter = asyncio.create_task(wait_heart_then_digicam_shutter(frame, serial_mica_handle, gcsWrites2Sony.set_mica_shutter.value))
            await doRedShutter
            if not (red_edge_shutter.set_value(gcsWrites2Sony.set_mica_shutter.value, red_edge_shutter.STATE_CAM_WRITING, 50) == True):
               print("\033[31m micasense shutter speed has not been successfully set \033[0m")   
               writeSuccess = False
               writeSuccess = gcsWrites2Sony.setVal_mica_shutter_sp(gcsWrites2Sony.set_mica_shutter.value, mavObj.STATE_CAM_WRITING, mavObj.WRITE_PREV_DATA, 50)
           
        if (not (gcsWrites2Sony.set_mica_mode.value == gcsWrites2Sony.STATE_INIT) and not (gcsWrites2Sony.set_mica_mode.value == gcsWrites2Sony.prev_mica_mode.value)) and (lockout == True):
            print(f"\033[35m on TOP LEVEL saw micaSEnse mode {gcsWrites2Sony.set_mica_mode.value} {gcsWrites2Sony.prev_mica_mode.value} {gcsWrites2Sony.mav_req_all_param.value} \033[0m")
            # send a heartbeat that says we are going into standby until we complete the camera action
            heartTask = asyncio.create_task(sendMavlinkHeartBeat_standby(frame, cID, 0))
            valid_modes = [ 1, 4 ]                                                                                         # although the menu has more modes to show we only can set these 2 modes 
            writeSuccess = False
            validSet = False
            for mod in range(0, len(valid_modes)):
                if (valid_modes[mod] == gcsWrites2Sony.set_mica_mode.value):            
                    doRedMode = asyncio.create_task(wait_heart_then_digicam_mode(frame, serial_mica_handle, gcsWrites2Sony.set_mica_mode.value, gcsWrites2Sony.set_mica_shutter.value))
                    await doRedMode
                    if not (red_edge_mode.set_value(gcsWrites2Sony.set_mica_mode.value, red_edge_shutter.STATE_CAM_WRITING, 50) == True):
                        writeSuccess = gcsWrites2Sony.setVal_mica_mode(gcsWrites2Sony.set_mica_mode.value, mavObj.STATE_CAM_WRITING, mavObj.WRITE_PREV_DATA, 50)
                        validSet = True
            if (validSet == False):
                writeSuccess = gcsWrites2Sony.setVal_mica_mode(gcsWrites2Sony.prev_mica_mode.value, mavObj.STATE_CAM_WRITING, mavObj.DONT_WRITE_PREV_DATA, 50)           
        ...
        if not (gcsWrites2Sony.cmd_wants_ack.value == True) and (gcsWrites2Sony.take_photo.value == 0):                                   # we have something to process and we have completed any photo taking
        ...
        # TODO :: class sends from sony camera class these should be in mavlink class 
        doMavReModeTask = asyncio.create_task(sendMavRedMode(mySonyCamNo1, red_edge_mode, cID)) 
        doMavRedShutTask = asyncio.create_task(sendMavRedShutter(mySonyCamNo1, red_edge_shutter, cID)) 
        
        # wait for completion of the mavlink senders        
        await doMavIsoTask
        await doMavAperTask
        await doMavFocusTask
        await doMavExproTask
        ...
        await doMavRedExpos 
        await doMavReModeTask 
        await doMavRedShutTask 
