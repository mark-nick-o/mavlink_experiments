...
#include "camera_image_captured_handler.hpp"
...
void SendCameraImageCapturedHandler::timerEvent(QTimerEvent* event)
{
    ...
    if ((m_model->get_substate() == DO_SENDING_ACK) && (m_model->get_sendState() == SENS_CIC))
    {
        std::uint16_t len=0u;
        ...
        */ 
        ...
        strcpy(com.file_url,"http://10.1.2.3/img/1.jpg"); /*<  URL of image taken. Either local storage or http://foo.jpg if camera provides an HTTP interface.*/
